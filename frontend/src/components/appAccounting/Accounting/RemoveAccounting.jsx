  ...
  return (
    <>
      <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginBottom: '10px' }}>
        <Typography>
          Удалить {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : ` ${accaunting.name}`}
        </Typography>
      </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginBottom: '10px' }}>
          <ThemeProvider theme={darkTheme}>
            <Typography>
              ...
              {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : ` ${accaunting.name}`}?
            </Typography>
          </ThemeProvider>
        </Box>
        {!errorEdit ? (

  ...
  return (
    <>
      <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginBottom: '10px' }}>
        <Typography>
          Удалить кабинет № {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : workplace.name}
        </Typography>
      </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-around', marginBottom: '40px' }}>
          <ThemeProvider theme={darkTheme}>
            <Typography>
              ...
              {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : workplace.name}?
            </Typography>
          </ThemeProvider>
        </Box>
        {!errorEdit ? (

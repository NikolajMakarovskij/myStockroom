  ...
  return (
    <>
      <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', marginBottom: '10px' }}>
        <Typography>
          Удалить кабинет № {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : mnf.name}
        </Typography>
      </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-around', marginBottom: '40px' }}>
          <ThemeProvider theme={darkTheme}>
            <Typography>
              ...
              {loading ? <LinearIndeterminate /> : error ? <PrintError error={error} /> : mnf.name}?
            </Typography>
          </ThemeProvider>
        </Box>
        {!errorEdit ? (
        ...
        <Box>
          <ThemeProvider theme={darkTheme}>
            <Box display='flex' justifyContent='space-between' alignItems='center' marginTop='20px'>
              ...
              <Button variant='contained' component={Link} to={`/manufacturer/list`}>
                Отмена
              </Button>
            </Box>
          </ThemeProvider>

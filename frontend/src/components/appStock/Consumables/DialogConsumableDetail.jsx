...
export default function DialogConsumableDetail(props) {
  ...
  const GetData = useCallback(async () => {
    try {
      await AxiosInstanse.get(`consumables/consumable_list/${id}/`).then((res) => {
        setDetail(res.data)
      })
    } catch (error) {
  ...
  return (
    <Fragment>
      ...
      <Dialog onClose={handleClose} fullWidth='true' maxWidth='sm' open={open}>
        <DialogTitle sx={{ m: 0, p: 2 }} id='consumables'>
...
DialogConsumableDetail.propTypes = {

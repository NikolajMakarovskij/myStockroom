...
def test_decom_add_devices(client):
    ...
    Decom.add_device_decom(device_id=devices.id, username=username, status_choice="В список на списание")
...
def test_stock_dev_remove_device(client):
    ...
    Stock.add_device(self=Stock(client), device=devices, quantity=quantity, number_rack=number_rack,
                     number_shelf=number_shelf, username=username)
    Decom.add_device_decom(device_id=devices.id, username=username, status_choice="В список на списание")
    Decom.remove_decom(device_id=devices.id, username=username, status_choice="Удаление")
...
def test_disp_add_devices(client):
    ...
    Stock.add_device(self=Stock(client), device=devices, quantity=quantity, number_rack=number_rack,
                     number_shelf=number_shelf, username=username)
    Decom.add_device_decom(device_id=devices.id, username=username, status_choice="В список на списание")
    Decom.add_device_disp(device_id=devices.id, username=username, status_choice="В список на утилизацию")
...
def test_disp_remove_device(client):
    ...
    Decom.add_device_decom(device_id=devices.id, username=username, status_choice="В список на списание")
    Decom.add_device_disp(device_id=devices.id, username=username, status_choice="В список на утилизацию")
    Decom.remove_disp(device_id=devices.id, username=username, status_choice="Удаление")
    test_history = HistoryDev.objects.get(status='Удаление')

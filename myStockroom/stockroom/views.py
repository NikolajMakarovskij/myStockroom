...
class stockroomView(DataMixin, generic.ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title="Склад расходников", searchlink='stockroom:stock_search', menu_categories=stock_cat )
...
class stockAccCategoriesView(DataMixin, generic.ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs ):
        ...
        if not cat_acc:
            ...
            cache.set('cat_acc', cat_acc, 300)
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title="Склад комплектующих", searchlink='stockroom:stock_acc_search', menu_categories=cat_acc)
        context = dict(list(context.items()) + list(c_def.items()))
        return context
...
class stockDevView(DataMixin, generic.ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title="Склад устройств", searchlink='stockroom:stock_dev_search', menu_categories=cat_dev)
        context = dict(list(context.items()) + list(c_def.items()))
        return context

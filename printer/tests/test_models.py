...
from ..models import Printer, Categories, Consumables
...
def test_printer_create():
    ...
    Consumables.objects.create(name = "fotoval") 
    Consumables.objects.create(name = "toner") 
    Consumables.objects.create(name = "fotodrumm") 
    printer = Printer.objects.create(  
        ...
        cartridge = Consumables.objects.get(name = "cartridge"),
        fotoval = Consumables.objects.get(name = "fotoval"),
        toner = Consumables.objects.get(name = "toner"),
        fotodrumm = Consumables.objects.get(name = "fotodrumm"),
        score = "0",
        note = "some_note"
    )  
    ...
    assert printer.cartridge.name == "cartridge"
    assert printer.fotoval.name == "fotoval"
    assert printer.toner.name == "toner"
    assert printer.fotodrumm.name == "fotodrumm"
